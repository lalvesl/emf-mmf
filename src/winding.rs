    ...
    if config.show_endwindings {
        ...
        for (i, assignment) in assignments.iter().enumerate() {
            ...
            if assign.direction == Direction::In {
                ...
                let rot1 = Quat::from_rotation_y(-slot_center + PI / 2.0) * Quat::from_rotation_y(PI / 4.0);
                let rot2 = Quat::from_rotation_y(-slot_center + PI / 2.0) * Quat::from_rotation_y(-PI / 4.0);
